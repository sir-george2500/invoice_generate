...
portPromise.then((port) => {
  ...
  expressApp.use('/', function (req, res, next) {
    ...
    let connectSrc = "";
    let scriptSrc = "'self' 'unsafe-inline' https://js.zohostatic.com";
    let styleSrc = "'self' 'unsafe-inline'";
    
    let manifest = fs.readFileSync(path.join(__dirname,"..","plugin-manifest.json")).toString();
    manifest = JSON.parse(manifest);
    
    if(manifest != null && manifest.cspDomains != null) {
      // Handle connect-src domains
      if(manifest.cspDomains["connect-src"] != null) {
      ...
      // Handle script-src domains
      if(manifest.cspDomains["script-src"] != null) {
        scriptSrc = manifest.cspDomains["script-src"].join(" ");
      }
      
      // Handle style-src domains  
      if(manifest.cspDomains["style-src"] != null) {
        styleSrc = manifest.cspDomains["style-src"].join(" ");
      }
    }
    
    const cspHeader = `connect-src https://*.zohostatic.com https://*.sigmausercontent.com ${connectSrc}; script-src ${scriptSrc}; style-src ${styleSrc}`;
